...
for run in range(runs):
    ...
    for nurse in instance.N:
        nurse.pref_alpha = round(
            np.random.uniform(0, 10, 1)[0]) / 10  # st.sidebar.slider('alpha*', 0.0, 1.0, 0.5, step=0.1)

    schedule, tot_dis, worst_off, coverage = find_schedule(instance, weight_over=10, weight_under=100,
                                                           include_satisf=True)  # returns NSP and sol
    schedule, tot_dis_BM, worst_off_BM, coverage_BM = find_schedule(instance, weight_over=10,
                                                                    weight_under=100,
                                                                    include_satisf=False)  # returns NSP and sol
